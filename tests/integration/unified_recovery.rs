...
/// 4. `recover_active_swap()` runs inside `do_coinswap()` — persists swapcoins,
...
fn test_unified_legacy_recovery_after_funding_broadcast() {
    ...
    // Swap balance may be non-zero: the taker's recover_active_swap() sweeps
...
/// 3. Taker "crashes" (DropAfterFundsBroadcast behavior fires).
/// 4. `recover_active_swap()` runs — contracts are already on-chain,
