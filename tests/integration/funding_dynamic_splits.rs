...
const TEST_CASES: &[(u64, &[u64], u64)] = &[
    (54_082, &[46824, 65658], 4),
    (102_980, &[100000, 107831], 4),
    (708_742, &[712971, 800000], 10),
    (500_000, &[298092, 712971], 4),
    (654_321, &[712971, 800000], 7),
    (90_000, &[91379, 100000], 10),
    (10_000, &[35892], 7),
    (1_000_000, &[1946436], 4),
    (999_999, &[1946436], 4),
    (123_456, &[298092], 5),
...
fn test_create_funding_txn_with_varied_distributions() {
    ...
    for (i, (target_amount, expected_inputs, expected_outputs)) in TEST_CASES.iter().enumerate() {
        ...
        // Assert the expected input UTXOs were selected.
