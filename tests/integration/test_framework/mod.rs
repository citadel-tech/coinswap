...
use coinswap::{
    maker::{UnifiedMakerBehavior, UnifiedMakerServer, UnifiedMakerServerConfig},
    protocol::common_messages::ProtocolVersion,
    taker::{UnifiedTaker, UnifiedTakerBehavior, UnifiedTakerConfig},
    utill::setup_logger,
...
#[allow(dead_code)]
pub fn wait_for_makers_setup(makers: &[Arc<UnifiedMakerServer>], timeout_secs: u64) {
...
impl TestFramework {
    ...
    pub fn init_unified(
        ...
        log::info!("🌐 Spawning local nostr relay on port {}", nostr_port);
        let (nostr_relay_shutdown, nostr_relay_handle) = spawn_nostr_relay(&temp_dir, nostr_port);

        _ = wait_for_relay_healthy(nostr_port);
    ...
    pub fn stop(&self) {
        ...
        // stop bitcoind and wait for it to exit before removing its data directory
        let _ = self.bitcoind.client.stop().unwrap();
        // Wait for bitcoind + nostr relay to fully release their ports
...
impl Drop for TestFramework {
    fn drop(&mut self) {
        // Ensure full cleanup even on panic — prevents orphaned processes and port collisions.
        self.shutdown.store(true, Relaxed);
        _ = self.nostr_relay_shutdown.send(());
...
fn wait_for_relay_healthy(port: u16) -> Result<(), String> {
    ...
    log::error!("Nostr relay not alive on port {}", port);

    Err(format!(
        "nostr relay did not become healthy on port {}",
        port
    ))
}
