...
fn test_taproot_per_hop_splits_1_3_1() {
    ...
    // Capture each maker's pre-swap spendable balance directly. (The shared
    // `verify_maker_pre_swap_balances` helper hardcodes a golden regular-balance value
    // tied to a 4-UTXO funding layout, which this test's funding does not match.)
    ...
    // A completed swap is the per-hop count assertion: the taker rejects any hop that does
    // not return exactly the agreed count, so success proves 1 -> 3 -> 1 held on-chain.
    taker
    ...
    // Structural assertions (exact sats depend on randomized chunking, so avoid golden
    // values): all contracts resolved, the taker paid a positive fee, and both makers
    // earned a positive service fee.
    assert_eq!(
    ...
    for (i, (maker, original_spendable)) in makers.iter().zip(maker_spendable_balance).enumerate() {
        ...
        info!("Maker {i} earned {} sats", earned.to_sat());
        assert!(
            earned > Amount::ZERO,
            "maker {} should have earned a fee",
            i
        );
    }
...
fn run_misbehaving_split_maker(behavior: MakerBehavior) {
    ...
    let (test_framework, mut takers, makers, block_generation_handle) =
        TestFramework::init(makers_config_map, taker_behavior, vec![behavior]);
...
fn test_taproot_uneven_split_old_maker_aborts_before_funding() {
    ...
    // Both hops of [1, 3, 1] are uneven (1->3 and 3->1), so every maker's missing support
    // is fatal regardless of which one the taker routes through first. With no spares to
    // substitute, the whole preparation aborts.
    let swap_params = SwapParams::new(ProtocolVersion::Taproot, Amount::from_sat(500_000), 2)
        .with_tx_counts(vec![1, 3, 1])
        .with_required_confirms(1);

    let result = taker.prepare_coinswap(swap_params);
    match result {
