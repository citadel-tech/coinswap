...
fn test_maker_cli() {
    ...
    assert_eq!(data_dir, "/tmp/coinswap/maker".to_string());
    ...
    assert_eq!(seed_balance, "1000000 SAT");
    assert_eq!(swap_balance, "0 SAT");
    assert_eq!(fidelity_balance, "5000000 SAT");
    assert_eq!(contract_balance, "0 SAT");
    ...
    // Check balances
    assert_eq!(maker_cli.execute_maker_cli(&["seed-balance"]), "999000 SAT");
    assert_eq!(maker_cli.execute_maker_cli(&["contract-balance"]), "0 SAT");
    assert_eq!(
        maker_cli.execute_maker_cli(&["fidelity-balance"]),
        "5000000 SAT"
    );
    assert_eq!(maker_cli.execute_maker_cli(&["swap-balance"]), "0 SAT");

    // Verify seed-utxo count
    let seed_utxo = maker_cli.execute_maker_cli(&["seed-utxo"]);
