...
fn fidelity_limit_violation() {
    ...
    // Attempt the swap - it will fail when taker discovers that maker's fidelity bond doesn't lie in the accepted fidelity_timelock limit via offerbook sync.
    match taproot_taker.do_coinswap(swap_params) {
        ...
        Ok(None) => {
            panic!("✅ Taproot coinswap triggered recovery was not expected (Ok(None)");
        }
        Err(e) => {
    ...
    match restart_result {
        Ok(_) => panic!("Maker should NOT start with invalid fidelity_timelock"),
        Err(e) => {
            info!("✅ Maker failed to start as expected: {:?}", e);
            assert!(
                format!("{:?}", e).contains("Invalid fidelity timelock"),
                "Unexpected error: {:?}",
                e
            );
        }
