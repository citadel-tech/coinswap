    ...
    for (i, _) in takers.iter().enumerate() {
        makers.iter().zip(org_maker_spend_balances.iter()).for_each(
            |(maker, org_spend_balance)| {
                ...
                let balance_diff = org_spend_balance.to_sat() - balances.spendable.to_sat();
                assert!(
                    balance_diff == 444642 // Successful coinswap for first maker
                        || balance_diff == 408142 // Successful coinswap for second maker
                        || balance_diff == 0, // No spending
