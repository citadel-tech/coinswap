...
impl UnifiedTaker {
    ...
    fn init_watch_service(
        ...
        let mut watcher =
            Watcher::<UnifiedTaker>::new(backend, registry, rx_requests, tx_events, nostr_relays);
    ...
    pub fn start_coinswap(&mut self, swap_id: &str) -> Result<UnifiedSwapReport, TakerError> {
        ...
        #[cfg(feature = "integration-test")]
        if self.behavior == UnifiedTakerBehavior::BroadcastContractAfterFullSetup {
    ...
    fn discover_makers(&mut self) -> Result<(), TakerError> {
        ...
        let (selected_makers, spares) = if let Some(addrs) = preferred {
            ...
            if self.offer_sync_handle.is_syncing() {
        ...
        match msg {
            MakerToTakerMessage::AckSwapDetails(ack) => {
                if let Some(tweakable_point) = ack.tweakable_point {
                    ...
                    #[cfg(feature = "integration-test")]
                    if self.behavior == UnifiedTakerBehavior::CloseAtAckResponse {
                        log::warn!(
    ...
    pub(crate) fn funding_reinitialize(&mut self) -> Result<(), TakerError> {
        ...
        {
            let mut wallet = self.write_wallet()?;
            let old_keys = wallet.unified_outgoing_keys_for_swap(&swap_id);
            for key in &old_keys {
                wallet.remove_unified_outgoing_swapcoin(key);
            }
            wallet.save_to_disk()?;
