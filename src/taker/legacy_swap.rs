...
use crate::{
    ...
    utill::{generate_keypair, generate_maker_keys, Bip324Stream, MIN_FEE_RATE},
    wallet::{
...
impl Taker {
    ...
    pub(crate) fn exchange_legacy(&mut self) -> Result<(), TakerError> {
        ...
        'exchange: while maker_idx < maker_count {
            ...
            let keepalive_stream = self.take_connection(maker_idx)?;
            ...
            let abort_check = || {
                ...
                if !breached && last_keepalive.get().elapsed() >= FUNDING_KEEPALIVE_INTERVAL {
                    if let Err(error) = &mut keepalive_stream.borrow_mut().send_message(
            ...
            self.return_connection(maker_idx, keepalive_stream.into_inner())?;
