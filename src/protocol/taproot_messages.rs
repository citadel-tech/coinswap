...
impl TryFrom<SerializableScalar> for Scalar {
    type Error = super::error::ProtocolError;

    fn try_from(scalar: SerializableScalar) -> Result<Self, Self::Error> {
        let bytes: [u8; 32] = scalar
            .0
            .try_into()
            .map_err(|_| super::error::ProtocolError::General("invalid scalar length"))?;
        Scalar::from_be_bytes(bytes)
            .map_err(|_| super::error::ProtocolError::General("invalid scalar value: out of range"))
    }
}
