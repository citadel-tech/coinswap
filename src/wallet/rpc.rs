...
impl Wallet {
    ...
    pub fn sync_and_save(&mut self) -> Result<(), WalletError> {
        let wallet_type = self.store.file_name.clone();
        log::info!("Initializing wallet sync and save for {wallet_type}");
        self.sync()?;
        self.save_to_disk()?;
        log::info!("Completed wallet sync and save for {wallet_type}");
        Ok(())
    }
