...
use bitcoin::{
    ...
    Address, Amount, OutPoint, PublicKey, Script, ScriptBuf, Sequence, Transaction, Txid, Witness,
};
use bitcoin::{TxIn, TxOut};
// use bitcoind::anyhow::Ok;
...
impl Wallet {
    ...
    pub fn coin_select(
        ...
        // Replace the TxIn construction with:
        let inputs: Vec<TxIn> = unspents
            ...
            .map(|(utxo, _)| TxIn {
                ...
                witness: match &utxo.witness_script {
                    Some(script) => {
...
//     #[test]
//     fn test_coin_selection() -> Result<(), WalletError> {
//         // Setup test wallet
//         let temp_dir = tempfile::tempdir()?;
//         let rpc_config = RPCConfig::from(test_framework.as_ref());
//         let wallet = Wallet::init(temp_dir.path().join("wallet.dat"), &rpc_config)?;

//         // Generate some test UTXOs by sending funds to wallet
//         let client = wallet.rpc;
//         let address = wallet.get_next_external_address()?;

//         // Create 3 UTXOs of different sizes
//         client.generate_to_address(101, &address)?; // Ensure coins are mature
//         client.send_to_address(&address, Amount::from_btc(1.0)?, None, None)?;
//         client.send_to_address(&address, Amount::from_btc(2.0)?, None, None)?;
//         client.send_to_address(&address, Amount::from_btc(1.5)?, None, None)?;
//         client.generate_to_address(1, &address)?; // Confirm transactions

//         // Test coin selection with different amounts
//         let test_amounts = vec![
//             Amount::from_btc(0.1)?,  // Small amount
//             Amount::from_btc(1.5)?,  // Medium amount
//             Amount::from_btc(3.0)?,  // Large amount requiring multiple UTXOs
//         ];

//         for amount in test_amounts {
//             let selected = wallet.coin_select(amount)?;

//             // Verify selection
//             let total_selected: Amount = selected.iter()
//                 .map(|(utxo, _)| utxo.amount)
//                 .sum();

//             assert!(total_selected >= amount,
//                 "Selected amount {} should be >= target {}",
//                 total_selected, amount);

//             // Verify no fidelity bonds were selected
//             assert!(selected.iter().all(|(_, spend_info)|
//                 !matches!(spend_info, UTXOSpendInfo::FidelityBondCoin { .. })),
//                 "Should not select fidelity bond UTXOs");
//         }

//         // Test error case - requesting more than available
//         let too_much = Amount::from_btc(10.0)?;
//         assert!(wallet.coin_select(too_much).is_err());
