...
impl Wallet {
    ...
    pub fn coin_select(
        ...
        if manually_selected_outpoints.is_some() {
            log::debug!(
                "Coin selection target: {} sats with {manually_selected_outpoints:?} manually selected UTXOs",
                amount.to_sat(),
            );
        }
        ...
        match select_coin(&single_output_groups, &coin_selection_option) {
            Ok(selection) => {
                ...
                log::debug!("Selected {} {utxo_type} UTXOs", final_selection.len());
                Ok(final_selection)
            }
            Err(e) => {
