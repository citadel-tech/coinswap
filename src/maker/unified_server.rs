...
pub fn start_unified_server(maker: Arc<UnifiedMakerServer>) -> Result<(), MakerError> {
    ...
    // Check for unfinished swapcoins from a previous run and start recovery.
    {
        let (inc, out) = maker
            ...
            .find_unfinished_unified_swapcoins();
        if !inc.is_empty() || !out.is_empty() {
            log::info!(
                "[{}] Incomplete unified swaps detected on startup: {} incoming, {} outgoing. Starting recovery.",
                maker.config.network_port,
                inc.len(),
                out.len()
            );
    ...
    let fidelity_handle = thread::Builder::new()
        .name("unified-fidelity-renewal".to_string())
        .spawn(move || {
