...
use super::{
    ...
    unified_handlers::{
        MakerConfig, SwapPhase, UnifiedConnectionState, UnifiedMaker as UnifiedMakerTrait,
    },
};
        ...
        if let Some(i) = highest_index {
            ...
            *proof = Some(highest_proof);
            ...
            let locktime = {
                ...
                let offset = if self.behavior == UnifiedMakerBehavior::InvalidFidelityTimelock {
...
impl UnifiedMakerTrait for UnifiedMakerServer {
    ...
    fn validate_swap_parameters(&self, details: &SwapDetails) -> Result<(), MakerError> {
        use super::unified_handlers::MIN_CONTRACT_REACTION_TIME;
            ...
            if details.timelock < MIN_CONTRACT_REACTION_TIME as u32 {
                return Err(MakerError::General(
                    format!(
                        ...
                        details.timelock, MIN_CONTRACT_REACTION_TIME
                    )
                    .leak(),
                ));
            }
